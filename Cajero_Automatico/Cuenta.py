...
def crear_cuenta(tipo_cuenta, usuario_id, saldo_inicial, numero_tarjeta, contrasena):
    ...
    database.cursor.execute('INSERT INTO cuentas (usuario_id, tipo_cuenta, saldo, numero_tarjeta, contrasena) VALUES (?, ?, ?, ?, ?)',
                   (usuario_id, tipo_cuenta, saldo_inicial, numero_tarjeta, contrasena))
...
def actualizar_cuenta(cuenta_id, nuevo_saldo, nueva_contrasena):
    ...
    database.cursor.execute('UPDATE cuentas SET saldo = ?, contrasena = ? WHERE id = ?', (nuevo_saldo, nueva_contrasena, cuenta_id))
    database.conn.commit()

def eliminar_cuenta(cuenta_id):
    # Elimina la cuenta de la base de datos
    database.cursor.execute('DELETE FROM cuentas WHERE id = ?', (cuenta_id,))
    database.conn.commit()

def buscar_cuenta(numero_tarjeta):
    # Busca la cuenta por número de tarjeta en la base de datos
    database.cursor.execute('SELECT * FROM cuentas WHERE numero_tarjeta = ?', (numero_tarjeta,))
